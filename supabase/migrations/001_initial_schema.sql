...
CREATE TABLE IF NOT EXISTS reports (
  ...
  status                    text NOT NULL DEFAULT 'active'
                              CHECK (status IN ('active','acknowledged','en_route','resolved','expired','flagged')),
  ...
  is_crime                  bool NOT NULL DEFAULT false,
...
CREATE TABLE IF NOT EXISTS reporter_profiles (
  ...
  accuracy_rate    float4 NOT NULL DEFAULT 0,
...
-- Atomic corroborate function (UPDATE...RETURNING is single atomic op, no race window)
...
BEGIN
  ...
  RETURNING confidence_score, corroboration_count;
...
CREATE INDEX IF NOT EXISTS idx_reports_parish_status ON reports(parish, status);
-- ivfflat for cosine similarity; set ivfflat.probes=10 at query time for good recall
...
-- Status updates: authority only, scoped to their parish
...
-- Authority users: readable only by authenticated authority users
CREATE POLICY "authority_read_users" ON authority_users FOR SELECT
  USING (auth.uid() IS NOT NULL AND EXISTS (SELECT 1 FROM authority_users WHERE id = auth.uid()));
